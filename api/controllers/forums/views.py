...
def create_thread(slug):
    ...
    if content.get("created") is None:
        ...
        command = '''INSERT INTO threads (forum_id, user_id, author, created, forum, message, slug, title)
				VALUES (%s, %s, '%s', '%s', '%s', '%s', '%s', '%s')	RETURNING id, author, created, forum, message, slug, title;''' % (
        forum["forum_id"], user["user_id"], user["nickname"],
    ...
    cursor.execute(INCREMENT_THREADS, [forum["forum_id"], ])
    data_context.put_connection(connect)
    cursor.close()
...
def get_list_of_users(slug):
    ...
    if since is not None:
        if desc:
            ...
            command = SELECT_USERS_SINCE_DESC % (forum["forum_id"], since, limit)
            cursor.execute(command)
        ...
        if desc:
            ...
            command = SELECT_ONLY_USERS % (forum["forum_id"], limit)
            cursor.execute(command)
            users = cursor.fetchall()
